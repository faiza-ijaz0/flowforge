...
void register_category_routes(httplib::Server& server,
  ...
  server.Get("/api/v1/categories", [category_repository](const httplib::Request& req,
                                                         httplib::Response& res) {
    // Mirrors GET /api/v1/products' pagination convention exactly (see
    // product_routes.cpp) -- capped at 200 for the same reason: a
    ...
    auto categories = category_repository->list(limit, offset);
    if (!categories) {
      write_error(res, categories.error());
      return;
    }
    auto total = category_repository->count();
    if (!total) {
    ...
    for (const auto& category : *categories) {
      items.push_back(to_json(category));
    }
    res.set_content(
        nlohmann::json{{"categories", items}, {"total", *total}, {"limit", limit}, {"offset", offset}}.dump(),
        "application/json");
  });
}
