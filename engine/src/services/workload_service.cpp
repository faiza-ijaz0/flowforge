...
Result<UserImportResult> WorkloadService::create_user_import_workload(const std::string& csv_content) {
  ...
  if (!parsed) {
    ...
    logger_->warn("workload_service", "user import rejected: invalid CSV",
  ...
  // handler, do not add another workload type yet) -- hardcoded here
  // rather than a `WorkloadService`->`handlers::` dependency just for one
  // string literal.
  ...
  auto created = create_workload(request);
  if (!created) {
    return std::unexpected(created.error());
  }

  if (metrics_) {
    metrics_->increment_counter("flowforge_user_imports_created_total");
    metrics_->increment_counter("flowforge_user_import_rows_total", static_cast<std::int64_t>(total_rows));
    metrics_->increment_counter("flowforge_user_import_rows_rejected_total",
                                static_cast<std::int64_t>(rejected_row_count));
  }
  logger_->info("workload_service", "user import created",
