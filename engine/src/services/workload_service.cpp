...
Result<domain::Workload> WorkloadService::with_progress(domain::Workload workload) const {
  ...
  // workload's job list.
  auto jobs = job_repository_->list_by_workload_id(workload.id(), workload.total_items());
  if (!jobs) {
  ...
  for (const auto& job : *jobs) {
    switch (domain::classify_job_status_for_workload(job.status())) {
      case domain::WorkloadItemOutcome::Completed:
        ++completed;
        break;
      case domain::WorkloadItemOutcome::Failed:
        ++failed;
        break;
      case domain::WorkloadItemOutcome::Active:
        break;
    }
  }
  workload.apply_progress(completed, failed);
  return workload;
}
